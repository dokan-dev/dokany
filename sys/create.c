    ...
    if (openRequiringOplock) {
      ...
      if ((status != STATUS_SUCCESS) &&
        ...
        DOKAN_LOG_FINE_IRP(RequestContext,
                      ...
                      status, DokanGetNTSTATUSStr(status), fcb, fcb->OpenCount, fcb->UncleanCount + 1);
...
VOID DokanCompleteCreate(__in PREQUEST_CONTEXT RequestContext,
  ...
  if (NT_SUCCESS(RequestContext->Irp->IoStatus.Status)) {
      DOKAN_LOG_FINE_IRP(RequestContext, "UncleanCount Inc FileObject=%p Fcb=%p Ccb=%p", RequestContext->IrpSp->FileObject, fcb, ccb);
     InterlockedIncrement(&fcb->UncleanCount);
